...
function SendLogin(form, type, popup) {
	...
	single_request = jQuery.ajax({
		'url': location.protocol + "//" + location.host + '/login.cgi',
		'method': 'POST',
		'data': dataQuery
	}).success(function(data) {
...
function getIconData() {
	$.ajax({method: 'GET',
		url: location.protocol + '//' + location.host + '/user/icon_desc.json?type=' + icons_data.page,
		dataType: 'json',
		async: false,
		cache: false
	}).done(function(data) {
		...
		if (dimmerable) {
			$(".slider").slider({
				...
				stop: function(event, ui) {
					...
					$.ajax({method: 'GET',
						url: location.protocol + "//" + location.host + '/user/action.cgi?dimmer=' + icon + '&val=' + ui.value,
						async: false,
						cache: false
					});
				}
...
function sendActionCommand(string) {
	$.ajax({method: 'GET',
		url: location.protocol + "//" + location.host + '/user/action.cgi?type=' + icons_data.page + '&' + string,
		async: false,
		cache: false
	});
	return false;
...
function sendClimaCommand(icon_num, action, setpoint, clima_override) {
	...
	var url = location.protocol + "//" + location.host + '/user/action.cgi?clima=' + icon_num + '&' + clima_type + '=' + action;
	if (setpoint) {
		url += '&val=' + setpoint;
	}
	$.ajax({
...
function setRGBColor(rgb, num_ico) {
	$.ajax({method: 'GET',
		url: location.protocol + "//" + location.host + '/user/action.cgi?rgb=' + num_ico + '&val=' + rgb,
		async: false,
		cache: false
	});
}
...
function createCounterGraph() {
	...
	if (page != '#' + view) {
		...
		counter.compare = [];
		counter.data = [];
		var csv_url = location.protocol + "//" + location.host + '/user/counter_data.csv?counter=' + page[3] + '&period=' + period + '&start=' + start;
		$.ajax({method: 'GET',
			url: csv_url,
			async: false,
			cache: false
			...
			if (page[3] != '200') {
				...
				var csv_url = location.protocol + "//" + location.host + '/user/counter_data.csv?counter=' + (parseInt(page[3]) + 1) + '&period=' + period + '&start=' + start;
				$.ajax({method: 'GET',
					url: csv_url,
					async: false,
					cache: false
...
function sendScenarioAction(string) {
	...
	if (icons_data.icon_protect[string] != '0') {
		$.ajax({method: 'GET',
			url: location.protocol + '//' + location.host + '/login.json',
			type: 'json',
			async: false,
			...
			if (!isLogged('alm', data)) {
				...
				$.ajax({method: 'GET',
					url: location.protocol + '//' + location.host + '/user/action.cgi?scenario=' + string,
					async: false,
					cache: false
				}).success(function(data) {
					if (data == '3') {
...
function sendConfigForm(form, type, callback) {
	...
	$.ajax({method: 'POST',
		url: location.protocol + '//' + location.host + '/setup/config.cgi?' + type,
		data: post_data}
	).done(function(data) {
		if (data != '0') {
...
function vedoIndexCreate() {
	$.ajax({method: 'GET',
		url: location.protocol + '//' + location.host + '/user/vedo_area_desc.json',
		dataType: 'json',
		async: false,
		cache: false
	}).done(function(data) {
...
function sendVedoCommand(type, area, force) {
	$.ajax({method: 'GET',
		url: location.protocol + "//" + location.host + '/user/action.cgi?' + (force == true ? 'force=1&' : '') + 'vedo=1&' + type + '=' + area,
		async: false,
		cache: false
	});
}
...
function vedoZoneStatAuth(data) {
	...
	if (logged) {
		...
		if (current != zone_stat_page) {
			$.ajax({method: 'GET',
				url: location.protocol + '//' + location.host + '/user/action.cgi?refresh=' + zone_stat_page.substring(1,zone_stat_page.length),
				async: true,
				cache: false
			});
...
function getVedoZoneData() {
	$.ajax({method: 'GET',
		url: location.protocol + '//' + location.host + '/user/vedo_zone_desc.json',
		dataType: 'json',
		async: false,
		cache: false
	}).done(function(data) {
...
//			stopPoll();
//			$.ajax({method: 'GET',
//				'url': location.protocol + '//' + location.host + '/user/vedo_area_desc.xml',
//				'dataType': 'xml',
//				'async': false
//			}).done(function(xml) {
//				var $data = $(xml);
